...
bot = telebot.TeleBot('7780895680:AAFsChif0G60ib2gVovRDNTtayM-ZAjFsdQ')
...
def add_user(message):
    ...
    if user_id in admin_id:
        ...
        response = "Mood ni hai abhi pelhe purchase kar isse:- @SHIVAxHEADSHOT."
...
 Usage: /remove <userid>'''
        ...
        response = "Purchase karle bsdk:- @SHIVAxHEADSHOT ."
...
def start_attack_reply(message, target, port, time):
    ...
    # Dictionary to store the last time each user ran the /SHIVA command
bgmi_cooldown = {}
...
@bot.message_handler(commands=['SHIVA'])
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:  # Updated to accept target, port, and time
            ...
            if time > 150:
                ...
                try:
                    record_command_logs(user_id, '/SHIVA', target, port, time)
            ...
            response = "Usage: /SHIVA <target> <port> <time>"
    else:
...
def show_help(message):
    help_text ='''
💥 /SHIVA : 😫BGMI WALO KI MAA KO CHODO🥵. 
